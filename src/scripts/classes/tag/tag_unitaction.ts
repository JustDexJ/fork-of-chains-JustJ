import type { TagMetadata } from "./TagHelper";

export const TAG_UNITACTION = {
  training: {
    type: "type",
    title: "Training",
    description: "Trains the slave in the art of servicing their betters",
  },
  purification: {
    type: "type",
    title: "Purification",
    description: "Restores a unit's body to its original and uncorrupted state",
  },
  corruption: {
    type: "type",
    title: "Corruption",
    description:
      "Corrupts the unit, replacing their bodyparts with something of demonic origin",
  },
  treatment: {
    type: "type",
    title: "Treatment",
    description: "Various procedures to heal a unit in both body and mind",
  },
  fleshshape: {
    type: "type",
    title: "Flesh-Shaping",
    description:
      "A collection of invasive procedure to alter the physique of a unit",
  },
} satisfies Record<string, TagMetadata>;
