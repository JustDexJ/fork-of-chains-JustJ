...
export namespace BanterHelper {
  ...
  export function banter(
    ...
    if (
      ...
      State.variables.friendship.adjustFriendship(
        initiator,
        target,
        friendship_amt,
      );
  ...
  export function isCanBanter(initiator: Unit, target: Unit) {
    ...
    if (
      target.isSlave() &&
      bedchamber &&
      bedchamber.isPrivate() &&
      bedchamber.getSlaver() != initiator
    )
      return false;
    ...
    if (!initiator.isHome() || !target.isHome()) {
      return (
        initiator.getTeam() &&
        target.getTeam() &&
        initiator.getTeam() == target.getTeam()
      );
    }
  ...
  export function _computeBanter(
    initiator: Unit,
    target: Unit,
    forced_sign?: number,
  ) {
    let compatibility = State.variables.friendship.getCompatibility(
      initiator,
      target,
    );
  ...
  export function doBanter(unit: Unit) {
    ...
    if (unit.isHome()) {
      let rooms = State.variables.bedchamberlist.getBedchambers({
        slaver: unit,
      });
    ...
    for (let i = 0; i < units.length; ++i) {
      let target = units[i];
      if (
        friends.length < setup.BANTER_USE_LIMIT &&
        target.getJob() == setup.job.slave
      )
        continue;
