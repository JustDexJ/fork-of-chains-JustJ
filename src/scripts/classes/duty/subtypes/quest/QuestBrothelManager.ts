...
import type { EventTemplateKey } from "../../../event/EventTemplate";
...
function brothelDescription(): DOM.Node {
  const outer_fragments = [];

  const duty = setup.dutytemplate
    .questbrothelmanager as DutyTemplateQuestBrothelManager;

  const progress = duty.progress();
  const classy = duty.class();
  const sub = duty.sub();
  const name = duty.brothelName();
  const gender = duty.gender();
  const profit = duty.profit();
  const net_profit = duty.netProfit();
  const favor = duty.favor();
  const facility = duty.facility();
  const attraction = duty.attraction();
  const promotion = duty.promotion();
  const high_income = duty.highIncome();

  const manager = setup.getUnit({ title: "quest_brothel_owner_0" as TitleKey });
...

  {
    ...
    fragments.push(html`
      ${setup.rng.choice([
        `Your brothel is located in the ${setup.lore["location_npc" as LoreKey].rep()},`,
        `Your brothel is situated within the ${setup.lore["location_npc" as LoreKey].rep()}`,
      ])}
      ${manager
        ? `and it is managed by ${manager.rep()}.`
        : `but nobody is managing it, and it will soon close...`}
    `);

    if (progress < 52) {
      fragments.push(html`
        ${setup.capitalize(name)} does not currently have any official name, and
        locals usually simply refer to it as "the brothel".
      `);
    } else {
      fragments.push(html`
        ${setup.rng.choice([
    ...
    if (progress < 8) {
      fragments.push(html`
        You have just acquired your brothel, and it's not yet operational right
        now.
      `);
    } else {
      let adj;
    ...
    if (progress >= 500) {
      ...
      fragments.push(html` ${setup.rng.choice(t)} `);
    }

    fragments.push(html`
      ${setup.rng.choice([
        html`On average, your brothel makes ${setup.DOM.Util.money(net_profit)}
        each week,`,
        html`Your brothel makes ${setup.DOM.Util.money(net_profit)} on an
        average week,`,
        html`The weekly profit of your brothel amounts to
        ${setup.DOM.Util.money(net_profit)},`,
      ])}
      ${!favor
        ? ""
        : html`
            ${setup.rng.choice([
              html`as well as ${setup.DOM.Util.favor(favor)} favor with the
              neko,`,
              html`in addition to ${setup.DOM.Util.favor(favor)} favor with the
              neko,`,
              html`together with ${setup.DOM.Util.favor(favor)} favor with the
              neko,`,
            ])}
          `}
      ${setup.rng.choice([
...

  {
    ...
    if (progress < 12) {
      fragments.push(html`
        Your brothel still occupies the same decrepit little hovel that were
        completely thrashed by the occupying bandits.
      `);
    } else if (progress < 76) {
      fragments.push(html`
        Your brothel is housed within a small non-descript building -- the very
        same one that was occupied by some bandits earlier, but it has been
        cleaned up and fixed to make it functional and well.
      `);
    } else if (progress < 116) {
      fragments.push(html`
        Your brothel occupies a large building after it has been expanded,
        complete with spacious rooms as well as
        ${facility == "rooms"
      ...
      let submissive;
      if (sub == "sub") {
        submissive = setup.rng.choice(["submissive"]);
      } else if (sub == "dom") {
        submissive = setup.rng.choice(["dominant"]);
      } else {
        submissive = setup.rng.choice(["skilled", "masterful", "expert"]);
      }
      ...
      fragments.push(html`
        ${setup.rng.choice(t)} ${setup.rng.choice(w)}
        ${setup.rng.choice([
  ...
  if (progress >= 8) {
    ...
    if (progress < 28) {
      ...
      if (classy > 0) {
        fragments.push(html`
          After a week of intense training led by one of your slavers, the
          ${whore}s now have basic knowledge in entertainment.
        `);
      } else {
        fragments.push(html`
          After a week of intense training led by one of your slavers, the
          ${whore}s now have basic knowledge in whoring.
        `);
      }
    } else if (progress < 100) {
      if (sub == "sub") {
        fragments.push(html`
          The ${whore}s are quite good at their job, and in particular in being
          a submissive partner to their clients.
        `);
      } else {
        fragments.push(html`
          The ${whore}s are quite good at their job, and in particular in
          sexually domiating their clients.
        `);
      }
    } else {
      fragments.push(html`
    ...
    if (progress < 12) {
      fragments.push(html`
        They sleep together in one of the few relatively clean rooms that
        remains within the establishment.
      `);
    } else if (progress < 76) {
      fragments.push(html`
        They sleep in pairs within the staff rooms available in the brothel,
        which has mostly been cleaned up from the questionable stains that
        belonged to their previous bandit occupants.
      `);
    } else if (progress < 116) {
      fragments.push(html`
        Each ${whore} now has their own personal room in the expanded brothel
        building.
      `);
    } else {
      fragments.push(html`
...
export class DutyTemplateQuestBrothelManager extends DutyTemplate {
  ...
  profit(): number {
    ...
    // high income period:
    if (this.extraHighIncome()) {
      base += 1500;
    } else if (this.highIncome()) {
      base += 750;
    }
  ...
  extraHighIncome(): boolean {
    return this.highIncome() >= EXTRA_HIGH_PROFIT_THRESHOLD;
  }

  highIncome(): number {
    const base = State.variables.varstore.get<string>(
      this.QUEST_BROTHEL_HIGH_INCOME,
    );
    if (!base) return 0;
    return parseInt(base);
  }

  decreaseHighIncome(): void {
    const high_income = this.highIncome();
    if (high_income >= EXTRA_HIGH_PROFIT_THRESHOLD) {
      ...
      State.variables.varstore.remove(this.QUEST_BROTHEL_HIGH_INCOME);
    }
  }

  progress(): number {
    const base = State.variables.varstore.get<string>(
      this.QUEST_BROTHEL_PROGRESS,
    );
    if (!base) return 0;
    return parseInt(base);
  }
  ...
  sub(): string {
    return State.variables.varstore.get(this.QUEST_BROTHEL_SUB)!;
  }

  promotion(): string {
    return State.variables.varstore.get(this.QUEST_BROTHEL_PROMOTION)!;
  }

  gender(): string {
    return State.variables.varstore.get(this.QUEST_BROTHEL_GENDER)!;
  }

  facility(): string {
    return State.variables.varstore.get(this.QUEST_BROTHEL_FACILITY)!;
  }

  attraction(): string {
    return State.variables.varstore.get(this.QUEST_BROTHEL_ATTRACTION)!;
  }
  ...
  brothelName(): string {
    ...
    if (progress < 3) {
      ...
      return State.variables.varstore.get<string>(this.QUEST_BROTHEL_NAME)!;
    }
  }

  randomizeBrothelName(): string {
    const name = `The ${generateTeamName()}`;
    this.setBrothelName(name);
    return name;
  }

  setBrothelName(name: string): void {
    State.variables.varstore.set(this.QUEST_BROTHEL_NAME, name, -1);
  }

  describe(): DOM.Node {
    return setup.DOM.Util.message(this.brothelName(), () => {
  ...
  resetChain() {
    return [
      ...
      setup.qc.SetCooldownQuest(
        "under_new_management" as QuestTemplateKey,
        300,
      ),
      ...
      setup.qc.SetCooldownEvent("pants_of_competence" as EventTemplateKey, 0),
      setup.qc.SetCooldownEvent("job_change" as EventTemplateKey, 0),
    ];
  }
  ...
  override advanceWeek(duty_instance: DutyInstance) {
    ...
    const owner = setup.getUnit({ title: "quest_brothel_owner_0" as TitleKey });
    const name = this.brothelName();
    ...
    if (!owner) {
      ...
      setup.notify(
        `Since a|rep is not a slaver under your employ, you could not benefit from the profits of ${name}`,
        { a: owner },
      );
    } else {
      // owner is present
      ...
      if (proc == "proc" || proc == "crit") {
        ...
        let wait_raw = State.variables.varstore.get<string>(
          this.QUEST_BROTHEL_WAIT,
        );
        if (wait_raw) {
          let wait = parseInt(wait_raw);
          if (wait == 1) {
            State.variables.varstore.remove(this.QUEST_BROTHEL_WAIT);
          } else {
            State.variables.varstore.set(this.QUEST_BROTHEL_WAIT, wait - 1, -1);
          }
        }
