...
function slaveOrderFilter(company: Company) {
  return (slave_order: SlaveOrder) => slave_order.getSourceCompany() == company;
}

function getSourceCompanyFilter() {
  const orders_sources = State.variables.slaveorderlist
    .getSlaveOrders()
    .map((order) => order.getSourceCompany());
  return Object.values(State.variables.company)
...
export const _MENUS_slaveorder: FilterMenu<SlaveOrder> = {
  source: {
    title: "Source",
    default: "All",
