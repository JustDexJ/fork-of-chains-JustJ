...
export abstract class SexLOcation_DungeonsLocationBase extends SexLocation {
  override getRestrictions() {
    return [setup.qres.Building("dungeons")];
  }
  ...
  override getFurnitures(): { [k in FurnitureSlotKey]: Furniture } {
    return {
      slaverbed: setup.item.f_slaverbed_other_dungeons_bench as Furniture,
      ...
      punishment: setup.item.f_punishment_none_dungeons as Furniture,
  ...
  override repRawAmbience(sex: SexInstance) {
    ...
    if (
      State.variables.company.player
        .getUnits()
        .filter((unit) => unit.isHasDick()).length
    ) {
      base.push(`the smell of cum continues to permeate the dungeons`);
    } else {
      base.push(`the smell of girlcum continues to permeate the dungeons`);
    }
    return base;
  }
}

export class SexLocation_DungeonsFloor extends SexLOcation_DungeonsLocationBase {
  constructor() {
    super(
      "dungeonsfloor",
      [
