export default class SexBodypartsCanInteract extends SexRestriction {
  constructor(
  ...
  override explain() {
    return `${this.my_actor_name}'s ${this.my_bodypart.repsimple()} can interact with ${this.their_actor_name}'s ${this.their_bodypart.repsimple()}`;
  }
  ...
  override isOk(action: SexAction) {
    ...
    if (!sex.isCanUse(me, this.my_bodypart)) return false;
    if (!sex.isCanUse(them, this.their_bodypart)) return false;
