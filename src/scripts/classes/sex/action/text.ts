 ...
 * @param past_sentences [sentences]: (x withdraws my dick, my dick slides out of their tongue, ..)
 * @param action_sentences What a sub want, e.g., (continue sucking on a|their cock)
 */
function afterPenetrationReaction(
  me: Unit,
  them: Unit,
  past_sentences: string[],
  action_sentences: string[],
  sex: SexInstance,
  is_dom?: boolean,
): string {
  ...
  if (theirpace == setup.sexpace.resist) {
    ...
    t = [` the attention`, ` everything, including when ${s}`];

    if (is_dom) {
      t = t.concat([`the abuse `]);
    } else {
      t = t.concat([`the pleasuring`]);
    }

    t = [setup.SexUtil.mindbrokenReactionNoun(them, sex, t)];
...
export default {
  ...
  /**
   * Me just finished penetrating them (as a dom). Give their reaction.
   * @param past_sentences [sentences]: (withdraws my dick, my dick slides out of their tongue, ..)
   * @param action_sentences What a sub want, e.g., (continue sucking on a|their cock)
   */
  afterPenetrationReactionDom(
    me: Unit,
    them: Unit,
    past_sentences: string[],
    action_sentences: string[],
    ...
    return afterPenetrationReaction(
      me,
      them,
      past_sentences,
      action_sentences,
   ...
   * @param past_sentences [sentences]: (withdraws their tongue, ..)
   * @param action_sentences What a sub/dom want, e.g., (continue having their tongue in their vagina)
   */
  afterPenetrationReactionSub(
    me: Unit,
    them: Unit,
    past_sentences: string[],
    action_sentences: string[],
   ...
   */
  hesitatesBeforeForcingThemselfTo(unit: Unit, sex: SexInstance) {
    ...
    return setup.SexUtil.convert(story, { a: unit }, sex);
  },

  /**
   * Me abusing b, who is a masochist.
   * @param actions (being spanked, being spit on their face, ...)
   */
  masochistReaction(
    me: Unit,
    them: Unit,
