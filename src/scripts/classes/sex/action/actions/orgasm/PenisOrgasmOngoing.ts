...
export class PenisOrgasmOngoing extends PenisOrgasm {
  getTags() {
    return super.getTags().concat([this.getPenetrationTarget().getTag()]);
  }
  ...
  describeOrgasm(sex: SexInstance): string {
    ...
    if (me.isHasTrait("dick_demon")) {
      modifiers.push(
        ` a|Rep a|continue to make small, thrusting movements, raking a|their barbs back against the inner walls of b|reps ${orifice} and causing b|them to let out b|a_moan.`,
      );
    } else if (me.isHasTrait("dick_dragonkin")) {
      if (me.isYou()) {
        modifiers.push(
          ` You feel your ribbed a|dick bumping against the inner walls of b|their ${orifice}, which causes b|them to let out b|a_moan.`,
        );
      } else {
        modifiers.push(
          ` The ribbed length of a|reps a|dick bumps against the inner walls of b|reps ${orifice}, which causes b|them to let out b|a_moan.`,
        );
      }
    }
    ...
    if (mypace == setup.sexpace.dom && theirpace != setup.sexpace.dom) {
      const floortrigger =
        ...
        (!sex.getUnitAtPosition(setup.sexposition.top) ||
          myposition == setup.sexposition.top) &&
        theirpose.isOnFloor();
      let text = `As a|rep a|feel b|reps ${orifice} squeezing down around a|their a|dick,
      ...
      text += `
        ...
        a|rep a|growl menacingly in b|their ear${
          sex.isCanTalk(me)
  ...
  cumTargetDescription(sex: SexInstance): string {
    const me = this.getActorUnit("a");
    const them = this.getActorUnit("b");
    ...
    if (them.isYou()) {
      ...
      t = [` deep into b|reps ${hole}.`, ` inside b|reps ${hole}.`];
    }
  ...
  postOrgasm(sex: SexInstance): string {
    const me = this.getActorUnit("a");
    const mypace = sex.getPace(me);
    ...
    if (mypace == setup.sexpace.dom && theirpace != setup.sexpace.dom) {
      ...
      if (
        ...
        mypose.getFacingHeight(this.getPenetratorBodypart(), myposition, sex)
          .height ==
          theirpose.getFacingHeight(
            this.getPenetrationTarget(),
            theirposition,
            sex,
          ).height
