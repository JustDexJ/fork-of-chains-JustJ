...
export function orgasmPositionPreparation(
  unit: Unit,
...
export function getCumQuantityDescription(unit: Unit, sex: SexInstance) {
  const cum = setup.SexUtil.traitSelectArray(unit, {
    ...
    balls_medium: [`a|their cum squirts`, `a|their cum splatters`],
    balls_large: [`a|their cum shoots out`],
    balls_huge: [`a|their cum spurts out`],
    balls_titanic: [`a|their cum blasts out`],
    default: [`not even a single drop of cum is produced...`],
  });
