...
export class CalmDown extends SexAction {
  getTags() {
    return super.getTags().concat(["nobodypart", "normal"]);
  }
    ...
    return [
      {
        ...
        restrictions: [
          setup.qres.SexArousalAtLeast(setup.SexConstants.AROUSAL_MIN_TRIGGER),
        ],
      },
    ];
  }
  ...
  getDiscomfortBase(unit: Unit, sex: SexInstance): number {
    ...
    if (!unit.isCanPhysicallyOrgasm()) {
      ...
      if (chance > 0) {
        base_discomfort += setup.SexConstants.DISCOMFORT_LARGE * chance;
      }
    }
