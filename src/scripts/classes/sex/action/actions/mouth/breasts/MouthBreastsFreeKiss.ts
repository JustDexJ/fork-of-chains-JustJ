...
export class MouthBreastsFreeKiss extends MouthBreastsFreeBaseDom {
  getTags() {
  ...
  getRestrictions() {
    return super
      .getRestrictions()
      .concat([setup.qres.HasItem("sexmanual_breasts_kiss")]);
  }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    if (them.isHasTrait("breast_titanic")) {
      ...
      !them.isHasTrait("breast_tiny") &&
      them.isHasTrait("muscle_strong")
    ) {
      t = [
