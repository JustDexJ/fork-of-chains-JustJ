...
export class NibbleNeck extends SexAction {
  getTags() {
    return super.getTags().concat(["dom", "mouth", "discomfort"]);
  }
  desc() {
    return "Nibble neck";
  }
  ...
  getDiscomfortMultiplier(
    ...
    if (actor_name == "a") {
      ...
      return setup.SexUtil.skinDiscomfortMultiplier(
        this.getActorUnit("b"),
        sex,
      );
    }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    let is_penetrating =
      setup.qres
        .SexIsOngoing(
          "a",
          setup.sexbodypart.penis,
          "b",
          setup.sexbodypart.vagina,
        )
        .isOk(this) ||
      setup.qres
        .SexIsOngoing("a", setup.sexbodypart.penis, "b", setup.sexbodypart.anus)
        .isOk(this);

    let t;
    if (is_penetrating) {
      const hole = sex
        .getBodypartPenetrationTarget(them, setup.sexbodypart.penis)!
        .bodypart.rep(me, sex);
      t = [
    ...
    let to, tp;
    const skin = setup.Text.Unit.Trait.skin(
      them,
      /* with eq = */ false,
      /* with adj = */ true,
    );
    ...
    if (them.isMasochistic()) {
      const base = setup.SexUtil.masochistReaction(
        me,
        them,
        [
      ...
      const base = setup.SexUtil.repResist(
        me,
        them,
        sex,
        [
