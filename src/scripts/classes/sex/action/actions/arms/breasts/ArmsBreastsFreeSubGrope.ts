...
export class ArmsBreastsFreeSubGrope extends ArmsBreastsFreeBaseSub {
  getTags() {
  ...
  getRestrictions() {
    return super
      .getRestrictions()
      .concat([setup.qres.HasItem("sexmanual_grope")]);
  }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    const hasclothes = sex.getCoveringEquipment(
      me,
    ...
    if (hasclothes) {
      clothesrep = sex
        .getCoveringEquipment(me, this.getPenetrationTarget())!
        .rep();
    } else {
    ...
    if (mypace == setup.sexpace.dom) {
      t = [
        ...
        letting out a|a_moan as a|they roughly a|press them into a|their a|breasts ${
          hasclothes
      ...
      t = [
        ...
        letting out a|a_moan as a|they a|eagerly a|press them into a|their a|breasts ${
          hasclothes
            ? `forcing a|their ${clothesrep} down against a|their a|nipples in the process`
            : ""
        }.`,
    ...
    if (hasbreasts) {
      ...
      adj = setup.rng.choice(["hard", "stiff", ""]);
    }
    ...
    if (theirpace == setup.sexpace.normal || theirpace == setup.sexpace.sub) {
      ...
      t = [
        setup.SexUtil.repResist(
          ...
          me,
          sex,
          [`struggle against b|their touch`, `disobey the order`],
          [
