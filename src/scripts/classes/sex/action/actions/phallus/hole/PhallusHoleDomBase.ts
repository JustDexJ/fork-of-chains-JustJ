...
export class PhallusHoleDomBase extends SexActionOnOngoingDom {
  ...
  getDiscomfortMultiplier(
    ...
    const my_size = this.getPenetratorBodypart().getSize(
      this.getActorUnit("a"),
      sex,
    );
    const their_size =
      this.getPenetrationTarget().getSize(this.getActorUnit("b"), sex) +
      this.getPenetrationTarget().getTraitSizeModifier(
        this.getActorUnit("b"),
        sex,
      );

    const size_diff = my_size - their_size;
    const idx = size_diff + setup.SexConstants.BODYPART_MAX_SIZE;
    ...
    if (actor_name == "a") {
      return (
        base *
        setup.SexConstants.BODYPART_SIZE_DIFFERENCE_DISCOMFORT_MULTIPLIER_GIVER[
          idx
        ]
      );
    } else {
      return (
        base *
        setup.SexConstants
          .BODYPART_SIZE_DIFFERENCE_DISCOMFORT_MULTIPLIER_RECEIVER[idx]
      );
    }
  }
  ...
  getArousalMultiplier(
    ...
    const their_size = this.getPenetrationTarget().getSize(
      this.getActorUnit("b"),
      sex,
    );
    ...
    if (actor_name == "a") {
      return (
        base *
        setup.SexConstants.BODYPART_SIZE_DIFFERENCE_AROUSAL_MULTIPLIER_GIVER[
          idx
        ]
      );
    } else {
      return (
        base *
        setup.SexConstants.BODYPART_SIZE_DIFFERENCE_AROUSAL_MULTIPLIER_RECEIVER[
          idx
        ]
      );
    }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    const myfacingheight = mypose.getFacingHeight(
      this.getPenetratorBodypart(),
      myposition,
      sex,
    );
    const theirfacingheight = theirpose.getFacingHeight(
    ...
    if (theirpace == setup.sexpace.resist) {
      ...
      theirpace == setup.sexpace.normal ||
      theirpace == setup.sexpace.sub
    ) {
      t = [
    ...
    story += " ";
    story += this.getPenetratorBodypart().repPenetrateFlavorSentence(
      me,
      them,
      this.getPenetrationTarget(),
      sex,
