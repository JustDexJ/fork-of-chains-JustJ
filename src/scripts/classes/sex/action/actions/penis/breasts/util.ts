 ...
 */
export function getDickPokeOutScore(
  me: Unit,
  them: Unit,
  sex: SexInstance,
): -2 | -1 | 0 | 1 | 2 {
  ...
  let breast_size;
  if (them.isHasTrait(setup.trait.breast_titanic)) {
    breast_size = 5;
  } else if (them.isHasTrait(setup.trait.breast_huge)) {
    breast_size = 4;
  } else if (
    !them.isHasBreasts() &&
    them.isHasTrait(setup.trait.muscle_extremelystrong)
  ) {
    breast_size = 4;
  } else {
  ...
  return Math.min(2, Math.max(-2, penis_size - breast_size)) as
    | -2
    | -1
    | 0
    | 1
 ...
 */
export function getDickPokeOutSentence(
  me: Unit,
  them: Unit,
  sex: SexInstance,
): string {
  ...
  // special case
  if (
    me.isHasTrait(setup.trait.dick_titanic) &&
    them.isHasTrait(setup.trait.breast_titanic)
  ) {
    t = [
