...
export class PenisBreastsOrgasmInside extends PenisBreastsOrgasmBase {
  getTags() {
    return super.getTags().concat(["normal"]);
  }
  desc() {
    return "Cum during titfuck / pecjob";
  }
  ...
  describeOrgasm(sex: SexInstance): string {
    ...
    let soft = "soft";
    if (
      setup.sexbodypart.breasts.getTitfuck(me, them) ==
      SexBodypart_Breasts.TITFUCK.pecjob
    ) {
      soft = "hard";
    }
