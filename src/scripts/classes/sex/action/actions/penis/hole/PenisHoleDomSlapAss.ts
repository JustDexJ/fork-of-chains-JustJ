...
export class PenisHoleDomSlapAss extends PenisHoleDomBase {
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    if (them.isMasochistic()) {
      t = [
        setup.SexUtil.masochistReaction(
          me,
          them,
          [
            `being spanked`,
      ...
      [setup.sexpace.forced, setup.sexpace.resist].includes(theirpace)
    ) {
      t = [
        ...
        ` The pained squeal which immediately escapes from b|reps b|mouth is exactly what
          ...
          ${them.isYou() ? `your eyes` : `a|their submissive bitch's b|eyes`}`,
      ...
      if (theirpace == setup.sexpace.sub) {
        ...
        w = [`protesting shout`, `pained exclamation`, `groan`, `angry squeal`];
      }
