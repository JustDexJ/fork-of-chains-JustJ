...
export class PenisHoleDomSpitroastTentacle extends PenisHoleDomBase {
  getTags() {
    return super.getTags().concat(["mouth"]);
  }

  getRestrictions() {
    return super
      .getRestrictions()
      .concat([setup.qres.HasItem("sexmanual_tentacle_spitroast")]);
  }
    ...
    return [
      {
        ...
        arousal: setup.SexConstants.AROUSAL_HUGE,
        paces: [setup.sexpace.dom, setup.sexpace.normal],
        restrictions: [setup.qres.Trait(setup.trait.magic_earth_master)],
      },
      {
        energy: setup.SexConstants.ENERGY_LARGE,
        ...
        restrictions: [setup.qres.SexCanUseBodypart(setup.sexbodypart.mouth)],
      },
    ];
  }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    const mouthfacingheight = theirpose.getFacingHeight(
