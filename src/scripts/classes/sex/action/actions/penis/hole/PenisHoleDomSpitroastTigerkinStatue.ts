...
export class PenisHoleDomSpitroastTigerkinStatue extends PenisHoleDomBase {
  ...
  getStatue(sex: SexInstance): Furniture {
  ...
  isStatueMale(sex: SexInstance): boolean {
    return this.getStatue(sex) == setup.item.f_object_master_tigerkinstatuemale;
  }

  repStatueDick(sex: SexInstance): string {
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    if (statuemale) {
      ...
      action = [`eat the tigerkin pussy`, `slather the tigerkin pussy`];
    }
