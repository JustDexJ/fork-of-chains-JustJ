...
function rawPenisMouthSizeDifference(
  me: Unit,
  them: Unit,
  sex: SexInstance,
  is_deepthroat: boolean,
): string {
  ...
  if (is_deepthroat) {
    fully = setup.rng.choice(["fully", "entirely"]);
    entire = setup.rng.choice([`entire`, `full`]);
    throat = "throat";
  ...
  let compat = setup.sexbodypart.penis.getAccomodatingValue(
    me,
    them,
 ...
 */
export function penisMouthSizeDifferenceDeep(
  me: Unit,
  them: Unit,
  sex: SexInstance,
) {
  const raw_story = rawPenisMouthSizeDifference(
    me,
    them,
    sex,
    /* is deep throat = */ true,
 ...
 * Describes unit sliding their cock inside their mouth all the way
 */
export function penisMouthSizeDifferenceRegular(
  me: Unit,
  them: Unit,
  sex: SexInstance,
) {
 ...
 * @param me  giving blowjob
 * @param them  receive blowjob
 */
export function blowjobReaction(me: Unit, them: Unit, sex: SexInstance) {
  ...
  if (thempace == setup.sexpace.resist) {
    ...
    thempace == setup.sexpace.normal ||
    thempace == setup.sexpace.sub
  ) {
    t = [
 ...
 * @param them - receive blowjob
 */
export function giveBlowjobReaction(me: Unit, them: Unit, sex: SexInstance) {
  const mypace = sex.getPace(me);
  const thempace = sex.getPace(them);
  ...
  if (thempace == setup.sexpace.resist) {
    ...
    if (them.isHasTrait("training_oral_advanced")) {
      t.push(
        ` a|They a|moan in pleasure as b|rep b|give an excellent head, caressing every part of the a|dick with b|their lips and b|their tongue.`,
      );
    }
  } else if (thempace == setup.sexpace.dom) {
    t = [
