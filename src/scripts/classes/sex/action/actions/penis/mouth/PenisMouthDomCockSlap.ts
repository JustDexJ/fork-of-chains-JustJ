...
export class PenisMouthDomCockSlap extends PenisMouthDomBase {
  getTags() {
  ...
  getRestrictions() {
    return super
      .getRestrictions()
      .concat([setup.qres.HasItem("sexmanual_cockaction")]);
  }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    if (unit.isHasTrait("dick_large")) {
      mid +=
        " " + setup.SexText.abuseImpression(unit, this.getActorUnit("b"), sex);
    }
