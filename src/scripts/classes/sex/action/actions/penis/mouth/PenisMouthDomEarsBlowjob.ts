...
export class PenisMouthDomEarsPull extends PenisMouthDomBase {
  getTags() {
    return super.getTags().concat(["dom", "discomfort"]);
  }
  desc() {
    return "Pull ears/horns and receive deep-throating";
  }
  ...
  isHorns(): boolean {
    const ears = this.getEars();
    return !!ears && ears.getTags().includes("horns");
  }
  ...
  getRestrictions() {
    return super
      .getRestrictions()
      .concat([setup.qres.HasItem("sexmanual_earsblowjob")]);
  }
  ...
  rawStory(sex: SexInstance): string | string[] {
    ...
    if (theirpace == setup.sexpace.normal || theirpace == setup.sexpace.sub) {
      ...
      if (them.isHasTrait("training_oral_advanced")) {
        dt.push(
          `As an experienced oral slut, b|rep b|have no problem at all swallowing the entire length of a|reps a|dick, comfortably sliding it up and down b|their throat.`,
        );
      } else if (them.isSlave()) {
        dt.push(
          `Despite the eagerness, the slave inexperience in oral sex draws chokes and uncomfortable moans from the b|race.`,
        );
      }

      if (them.isSlaver()) {
        dt.push(
          `The rough treatment causes the slaver to b|eagerly moan in slight embarrassment, muffled by the a|dick still lodged inside b|their throat.`,
        );
      }
      ...
      theirpace == setup.sexpace.resist ||
      theirpace == setup.sexpace.forced
    ) {
      dt = [
