...
function rawAnalEnjoymentDescription(
  me: Unit,
  sex: SexInstance,
): string | string[] {
  ...
  const enjoyment = SexBodypart_Anus.unitAnalEnjoymentMultiplier(me);
  if (enjoyment >= 2) {
    return [
...
export function analEnjoymentDescription(me: Unit, sex: SexInstance): string {
  return setup.SexUtil.convert(
    rawAnalEnjoymentDescription(me, sex),
    { a: me },
    sex,
  );
}
