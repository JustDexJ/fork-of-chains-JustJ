 ...
 */
export default {
  preThought(unit: Unit, sex: SexInstance): string {
    ...
    if (pace == setup.sexpace.mindbroken) {
      ...
      choice = [`With a a|adjper smirk`, `Grinning a|adv`];
    }
   ...
   */
  moan(unit: Unit, sex: SexInstance): string {
    let choices;
    if (!sex.isCanUse(unit, setup.sexbodypart.mouth)) {
      ...
      choices = [`audible moan`, `moan`, `groan`, `audible groan`];
    }
   ...
   * sobs etc
   */
  sob(unit: Unit, sex: SexInstance): string {
    let choices;
    if (!sex.isCanUse(unit, setup.sexbodypart.mouth)) {
      choices = [`muffled sob`, `muffled whine`, `gagged sob`, `gagged whine`];
    } else {
      choices = [`audible sob`, `sob`, `whine`, `audible whine`];
    }
    return setup.rng.choice(choices);
  },
   ...
   */
  postThought(unit: Unit, sex: SexInstance): string {
    const pace = sex.getPace(unit);
