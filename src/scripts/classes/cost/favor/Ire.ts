...
export default class Ire extends Cost {
  ...
  override apply(context?: CostContext) {
    ...
    if (adjusted > 0) {
      setup.notify(
        `Gain ${setup.DOM.toString(setup.DOM.Text.dangerlite(adj))} ire with ${company.rep()}`,
      );
    } else if (adjusted < 0) {
      setup.notify(
        `Lose ${setup.DOM.toString(setup.DOM.Text.successlite(adj))} ire with ${company.rep()}`,
      );
    }
  }
