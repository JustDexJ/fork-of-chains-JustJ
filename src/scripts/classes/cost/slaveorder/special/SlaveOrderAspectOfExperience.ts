import { SlaveOrderTemplate } from "../SlaveOrderTemplate";

export default class SlaveOrderAspectOfExperience extends SlaveOrderTemplate {
  constructor() {
    super();

    this.base_price = 0;
    this.trait_multi = 3000;
    this.value_multi = 0;

    this.name =
      "Order for a personal slave for a chieftain in the Northern Vale";
    this.company_key = "humanvale";
    this.expires_in = 10;
    this.fulfilled_outcomes = [];
    this.unfulfilled_outcomes = [];
    this.destination_unit_group_key = "soldslaves";
  }

  override text(): string {
    return `setup.qc.SlaveOrderAspectOfExperience(${this.value_multi})`;
  }

  override getCriteria(quest: QuestInstance) {
    const critical: Trait[] = [];
    const disaster: Trait[] = [];

    const unit = quest.getActorUnit("slave");
    const traits = unit.getTraits();
    for (const trait of traits) {
      if (
        trait.isAttachable() &&
        !trait.getTags().includes("race") &&
        !trait.getTags().includes("subrace") &&
        !trait.getTags().includes("training")
      ) {
        critical.push(trait);
      }
    }

    const req = [setup.qres.Job("slave"), setup.qres.NoTrait(unit.getRace())];

    let criteria = new setup.UnitCriteria(
      null /* key */,
      "Chieftain Order" /* title */,
      critical,
      disaster,
      req,
      {} /* skill effects */,
    );
    return criteria;
  }
}
