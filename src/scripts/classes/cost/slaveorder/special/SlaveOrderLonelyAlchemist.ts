import SlaveOrderItem from "../SlaveOrderItem";

export default class SlaveOrderLonelyAlchemist extends SlaveOrderItem {
  constructor(item: Item | ItemKey) {
    super(
      item,
      Math.floor(25000 / resolveObject(item, setup.item).getValue()!),
    );
  ...
  override getCriteria() {
    ...
    const critical = setup.rng
      .choicesRandom(crit_traits_pool, 2)
      .reduce((a, b) => a.concat(b), []);
    const disaster = setup.rng
      .choicesRandom(disaster_traits_pool, 2)
      .reduce((a, b) => a.concat(b), []);
