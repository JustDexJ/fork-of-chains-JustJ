...
export default class GenName extends Cost {
  trait_keys: TraitKey[];
  gender_found: boolean;
  subrace_found: boolean;

  constructor(
    public actor_name: string,
    public name_traits: Trait[],
  ) {
    super();
    ...
    for (let i = 0; i < name_traits.length; ++i) {
      if (!name_traits[i].key)
        throw new Error(
          `${name_traits[i]} at ${i}-th position not found for genname`,
        );
