...
function get_equipment_pool_callback(pool: EquipmentPool): () => void {
  return () => {
    const company = State.variables.company.kobold.rep();
    let t = [
...
function get_random_trinket_good(value_average: number): () => void {
  return () => {
    const company = State.variables.company.kobold.rep();
...
function get_random_trinket_bad(value_average: number): () => void {
  return () => {
    const company = State.variables.company.kobold.rep();
    const value_actual = setup.nudgeMoney(value_average);
    let t = [
 ...
 */
export default class RandomDeeprealmItem extends Cost {
  constructor(public value: number) {
    super();

    if (value < 500) {
      throw new Error(
        `Minimum value for random deeprealm item is 500g, but received ${value} instead`,
      );
    }
  }
  ...
  override apply(context: CostContext) {
    ...
    if (this.value >= 1500) {
      ...
      pools.push([get_dwarven_axe, 0.01]);
      avg.push([axe.getValue(), 0.01]);
    }
