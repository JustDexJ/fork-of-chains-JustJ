...
export namespace UnitTitleHelper {
  export const UNIT_TITLE_SLAVE_NO_TRAINING = "Defiant Slave";

  export const UNIT_TITLE_NONE = "Generalist";

  export const UNIT_TITLE_MAP_SINGLE = [
    "Fighter" /* combat */,
    "Brawler" /* brawn */,
    "Scout" /* survival */,
    "Spy" /* intrigue */,
  ...
  export const UNIT_TITLE_MAP_DOUBLE = [
    [
      "" /* combat combat */,
   ...
   * @param include_will If true, will return will trait if the slave is still unbroken instead.
   */
  export function getMainTraining(unit: Unit, include_will?: boolean): Trait {
    if (unit.isMindbroken()) return setup.trait.training_mindbreak;

    let trainings = unit.getAllTraitsWithTag("training");
    if (!trainings.length) {
    ...
    let maxval = Math.max.apply(
      Math,
      trainings.map(function (trait) {
        return trait.getSlaveValue();
      }),
    );
    let eligible = [];
  ...
  export function getTitleSlave(unit: Unit): string {
    let training = getMainTraining(unit, /* include will =*/ true);
    return training.text().title ?? "???";
  }

  export function getTitleSlaver(unit: Unit): string {
    let level = unit.getLevel();
    let targetstat = 8 + level * 1.1;
    let skills = unit.getSkillsBase();
    let high = [];
    ...
    if (!high.length || high.length > 4) return UNIT_TITLE_NONE;
    if (high.length == 1) {
      return UNIT_TITLE_MAP_SINGLE[high[0]];
    }
    ...
    const base =
      UNIT_TITLE_MAP_DOUBLE[Math.min(high[0], high[1])][
        Math.max(high[0], high[1])
      ];
    if (high.length == 2) return base;
    if (high.length == 3) {
      return `${base}-${UNIT_TITLE_MAP_SINGLE[high[2]]}`;
    }

    const suffix =
      UNIT_TITLE_MAP_DOUBLE[Math.min(high[2], high[3])][
        Math.max(high[2], high[3])
      ];
    return `${base}-${suffix}`;
  }

  export function getTitle(unit: Unit): string {
