...
import { Args_OneActor, MacroUtil } from "../macro/_metadata.js";

// Predefined internal data structure to speed up lookup. The order is:
// subjective, objective, dependent possessive, independent possessive,
// reflexive + capitalized variants
//
// prettier-ignore
...
export const getBasePronoun = (character: Unit, index: number) => {
...
export const getRelevantCharacter = (character: Unit | UnitKey): Unit => {
  return resolveObject(character, State.variables.unit);
};
...
function internalOutput(
  ...
  output.appendChild(
    document.createTextNode(
      pronouns[getBasePronoun(getRelevantCharacter(character), index)][index],
    ),
  );
}

/** Helper which also registers the macro metadata into MACROS_METADATA */
function Macro_add(name: string, def: string | number) {
  if (typeof def === "string") {
    // alias
    Macro.add(name, def as any);
    MacroUtil.registerMetadata(name, def);
  } else {
    const index = def;
    Macro.add(name, {
      handler() {
        internalOutput(this.output, this.args[0], index);
      },
    });
    MacroUtil.registerMetadata(name, {
...
Macro.add("pronounload", {
  handler() {
    ...
    let obj = this.args[0];
    let unit = this.args[1];

    setup.DOM.pronounload(obj, unit);
  },
});
