...
function addOverride(article: string, word: string, caseSensitive?: boolean) {
  ...
  if (State.length > 0) {
    ...
    msg =
      "cannot add article override -> needs to be run in StoryInit special passage or earlier";
    throw new Error(msg);
  }
  // check args
...
function _checkVowels(word: string) {
  ...
  return (
    _isDefaultIrregular(word, article) || _isAcronym(word, article) || article
  );
}
