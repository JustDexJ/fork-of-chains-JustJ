export const DOM_Menu_opportunityautoanswer = function (): DOM.Node {
  const fragments: DOM.Attachable[] = [];

  // flavor texts
  {
    const vice_leader =
      State.variables.dutylist.getUnitIfAvailable("viceleader")!;

    fragments.push(html`
      <p>
        You went over to ${vice_leader.rep()}'s desk. The following is the list
        of all mails that ${vice_leader.rep()} will automatically responds to.
      ...
      fragments.push(html`
        <div>
          ...
          ${setup.DOM.Util.include(
            opportunity.getOptions()[index].description_passage,
          )}
