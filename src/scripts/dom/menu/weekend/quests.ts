...
function displayEndweekQuest(quest: QuestInstance): {
  node: DOM.Node;
  notifications: string[];
} {
  const outer = [];
  ...
  fragments.push(html`
    ...
    ${setup.DOM.Util.namebold(quest)}:
    ${setup.DOM.Util.outcome(quest.getOutcome()!)}
  `);
  if (quest.getOutcome() == "failure") {
    fragments.push(
      setup.DOM.Util.help(html`
        Failure can be beneficial. Your slavers learn much better from failures,
        and as a result they gain a large amount of experience from failures,
        which helps them level up faster.
      `),
    );
  } else if (quest.getOutcome() == "disaster") {
    fragments.push(
      setup.DOM.Util.help(html`
        While disaster outcomes usually entail some bad consequences, it also
        have its benefits. Slavers learn a huge amount from life-threatening
        experiences, and as a result they gain a massive amount of experience
        from disaster results. What does not kill you makes you stronger, after
        all.
      `),
    );
  }
  fragments.push(html` by ${quest.getTeam()!.rep()}. `);
  ...
  fragments.push(html`
    ${setup.DOM.Nav.link(`(toggle full quest results)`, () => {
