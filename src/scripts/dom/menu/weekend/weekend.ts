 ...
 */
export const DOM_Menu_weekend = function (): DOM.Node {
  // turn off menu
  ...
  if (State.variables.calendar.getWeek() == 1 && !State.variables.gDebug) {
    ...
    setup.notify(
      `You must build the ${setup.buildingtemplate.scouthut.rep()} to proceed`,
    );
    return setup.DOM.Util.include("QuestHub");
  }
...
function autoSave() {
  if (
    State.variables.settings.shouldAutosave(State.variables.calendar.getWeek())
  ) {
...
function doWeekend(): DOM.Node {
  ...
  fragments.push(html`
    <div>${setup.DOM.Nav.move(`Continue`, next_passage)}</div>
  `);
 ...
 */
export const DOM_Menu_gameinit = function (): DOM.Attachable {
  autoSave();
  ...
  if (State.variables.gDebug) {
    ...
    return html`
      <div>${setup.DOM.Nav.button(`Begin the game`, () => {}, "QuestHub")}</div>
    `;
  }
};
 ...
 */
export const DOM_Menu_debugComputeStats = function (): void {
  autoSave();
  printDebugInfos();
};
