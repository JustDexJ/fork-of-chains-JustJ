...
export const DOM_Menu_relationsofficemanagedcompanies = function (): DOM.Node {
  ...
  fragments.push(html`
    <div>
      Select the companies for your ${manager.rep()} to manage their favors.
      When managed, their favors will no longer decay when below
      ${setup.DOM.Text.successlite("85.0")} favor. Their decay will also be much
      slower when the favor is above ${setup.DOM.Text.successlite("85.0")}.
      <p />
    </div>
  `);
  ...
  if (unit && unit.isAvailable()) {
    fragments.push(html`
      <div>
        ...
        ${setup.DOM.PronounYou.They(unit)} is able to manage ${max_managed}
        companies right now. That means only the first ${max_managed} of the
        companies below will have their relationship decay reduced.
    ...
    fragments.push(html`
      <div>
        ${setup.DOM.Util.YourRep(unit)} usually works here, but
        ${setup.DOM.PronounYou.they(unit)} is currently unavailable to attend to
        ${setup.DOM.PronounYou.their(unit)} duties.
        <p />
      </div>
    `);
  } else {
  ...
  fragments.push(html`
    <div>
      ${setup.DOM.Nav.link(`(Finish changing managed companies)`, () => {
  ...
  if (all_managed_companies.length < max_managed) {
    const companies_to_manage: Company[] = [];
    for (const [companykey, company] of objectEntries(
    ...
    fragments.push(html`
      <div>
        ${setup.DOM.Util.message(`(Manage new company)`, () => {
          const companies_to_manage_fragments: DOM.Node[] = [];
          ...
          if (companies_to_manage_fragments.length) {
            return setup.DOM.create("div", {}, companies_to_manage_fragments);
          } else {
            return null;
          }
        })}
  ...
  setup.DOM.Nav.topLeftNavigation(
    setup.DOM.Nav.move(`Finish [space]`, "RelationsOffice"),
  );
