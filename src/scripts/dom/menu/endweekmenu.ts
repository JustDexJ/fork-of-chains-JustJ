...
function marketObjectsDescriptor(market: Market): string | null {
...
function showAllNextEvents(): DOM.Node {
...
function getNextEventsMenu(): DOM.Attachable {
  ...
  if (!next_events) {
    ...
    fragments.push(
      html`In ${next_events.occur_week - State.variables.calendar.getWeek()}
      wk${next_events.events.length > 1 ? "s" : ""}: `,
    );
  ...
  return setup.DOM.Nav.link(setup.DOM.create("div", {}, fragments), () => {
    setup.Dialogs.open({
...
function getInfoMenu(): DOM.Node {
  ...
  if (State.variables.calendar && State.variables.unit?.player) {
    ...
    if (
      ...
      State.variables.fort.player.isHasBuilding("noticeboards")
    ) {
...
function getTopLeftNavigationMenu(): DOM.Node | null {
    ...
    if (State.variables.opportunitylist.isHaveMustAnswerOpportunities()) {
      ...
      fragments.push(
        setup.DOM.Util.help(html`
          You have some ${setup.DOM.Text.danger("important")} mail that must be
          answered before you can end the week.
        `),
      );
    } else {
      ...
      if (!deployable) {
        fragments.push(
          setup.DOM.create(
            "span",
            { class: "press_on_space" },
            setup.DOM.Nav.link(html`END WEEK`, endWeek),
          ),
        );
      } else {
        ...
        const ele = setup.DOM.create(
          ...
          setup.DOM.Nav.link(html`END WEEK`, () => {
            setup.DOM.Helper.replace(
              `#${menu_id}`,
              setup.DOM.Nav.link(html`REALLY END WEEK`, endWeek),
            );
          }),
        );
        fragments.push(ele);
 ...
 */
export const DOM_Menu_endweekmenu = function (): DOM.Node {
  const fragments: DOM.Attachable[] = [];
