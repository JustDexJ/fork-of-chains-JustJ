type SexProposeResult = { acceptance: string; dialogue: string };

function proposeSexSlave(unit: Unit): SexProposeResult {
  let dialogue;
  if (!unit.isCanTalk()) {
    if (unit.isObedient()) {
...
function proposeSexSlaver(unit: Unit): SexProposeResult {
  const speech = unit.getSpeech();

  const sugar = setup.Text.Greeting.nickname({
    unit: unit,
    target: State.variables.unit.player,
  });
  ...
  if (speech == setup.speech.friendly) {
    ...
    question = [`Sex? With you?`, `${heyboss} sex?`, `${heyboss} fun times?`];
  ...
  if (unit.getLover() == State.variables.unit.player) {
    ...
    if (speech == setup.speech.friendly) {
      ...
      response = [`I don't know you.`, `I don't have sex with strangers.`];
    } else if (speech == setup.speech.witty) {
      response = [
  ...
  if (acceptance == "yes" && !response) {
    if (speech == setup.speech.friendly) {
      ...
      response = [`...why not.`, `Fine, let's go.`, `Sure.`];
    } else if (speech == setup.speech.witty) {
      response = [
  ...
  const pre = setup.Text.replaceUnitMacros(question, {
    a: unit,
    b: State.variables.unit.player,
  });
  const post = setup.Text.replaceUnitMacros(response, {
    a: unit,
    b: State.variables.unit.player,
  });
  return {
 ...
 */
export const DOM_Menu_proposeSex = function (unit: Unit): DOM.Node {
  ...
  if (response.acceptance == "yes") {
    State.variables.gInteractiveSexUnitIds = [
      State.variables.unit.player.key,
      unit.key,
    ];
    ...
    setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.return("Return"));
    fragments.push(html` <div>${setup.DOM.Nav.return("(return)")}</div> `);
  } else {
