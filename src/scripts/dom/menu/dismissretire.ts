export const DOM_Menu_dismissConfirm = function (unit: Unit): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html`
    <div>
      Are you sure you want to ${setup.DOM.Text.danger("DISMISS")}
      ${unit.rep()}?
  ...
  fragments.push(html`
    <div>
      ${setup.DOM.Nav.button(`Yes, I want to DISMISS ${unit.rep()}`, () => {
...
export const DOM_Menu_retireConfirm = function (unit: Unit): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html`
    <div>
      Are you sure you want to ${setup.DOM.Text.dangerlite("retire")}
      ${unit.rep()}?
      ${setup.Text.replaceUnitMacros(
  ...
  fragments.push(html`
    <div>
      ${setup.DOM.Nav.button(`Yes, I want to retire ${unit.rep()}`, () => {
...
export const DOM_Menu_retireAftermath = function (unit: Unit): DOM.Node {
  const fragments: DOM.Attachable[] = [];

  function parse(texts: string[]): string {
  ...
  // dismiss paragraph
  {
    ...
    fragments.push(html` <p>${para.join(" ")}</p> `);
  }
  ...
  {
    // livings info
    fragments.push(html` <p>${setup.DOM.Card.livingdescription(unit)}</p> `);
  }

  setup.DOM.Nav.topLeftNavigation(setup.DOM.Nav.move(`Continue`, `Lodgings`));

  fragments.push(html`
    <div>${setup.DOM.Nav.move(`Continue`, `Lodgings`)}</div>
  `);

  return setup.DOM.create("div", {}, fragments);
};

export const DOM_Menu_retiredDismissConfirm = function (unit: Unit): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html`
    <div>
      Are you sure you want to ${setup.DOM.Text.danger("REMOVE")} ${unit.rep()}
      permanently?
      ${setup.Text.replaceUnitMacros(
    ...
    <div>
      ${setup.DOM.Nav.button(
        ...
        () => {
          ...
          setup.notify(`a|Rep a|have been removed from the game`, { a: unit });
          setup.DOM.Nav.gotoreturn();
        },
      )}
    </div>
...
export const DOM_Menu_rehireRetiredConfirm = function (unit: Unit): DOM.Node {
  ...
  fragments.push(html`
    <div>
      Are you sure you want to ${setup.DOM.Text.successlite("re-hire")}
  ...
  fragments.push(html`
    <div>
      ${setup.DOM.Nav.button(`Yes, I want to re-hire ${unit.rep()}`, () => {
