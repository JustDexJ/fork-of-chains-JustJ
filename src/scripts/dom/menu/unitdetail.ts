...
export const DOM_Menu_unitdetail = function (unit: Unit): DOM.Node {
  const outer = [];
  outer.push(html` <div>${getBackButton(unit)}</div> `);
  ...
  outer.push(html`
    <div>${setup.DOM.Menu.unitdescription(unit)}</div>
    <hr />
  `);
  ...
  if (State.variables.fort.player.isHasBuilding("armory")) {
    ...
    let actions: DOM.Attachable = null;
    if (unit.isCanChangeEquipmentSet()) {
      actions = setup.DOM.Nav.link(`(change)`, () => {
