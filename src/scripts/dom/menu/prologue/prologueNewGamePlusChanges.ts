...
function traitCallback(trait?: Trait | null) {
  return () => {
    if (trait) {
 ...
 */
export const DOM_Menu_prologueNewGamePlusPCChange = function (): DOM.Node {
  const choices: Array<[Trait | null, string, string]> = [];
  ...
  const traits_basic = basic_tags.map((tag) =>
    TraitHelper.getAllTraitsOfTags([tag]),
  );
  for (const trait of traits_basic.flat(1)) {
  ...
  const fragments = choices.map(
    (choice) => html`
      <div>
        ${setup.DOM.Nav.link(choice[2], traitCallback(choice[0]))}
        (${choice[1]})
      </div>
    `,
  );
