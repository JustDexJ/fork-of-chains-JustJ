...
export const DOM_Menu_settingsbase = function (): DOM.Node {
    ...
    fragments.push(html`
      <div>
        ...
        ${setup.DOM.Util.help(html`
          ...
          auto-save, and 1 to auto-save every week. Larger number will make the
          end-of-week processing faster.
        `)}
      </div>
    `);
  }
  ...
  fragments.push(
    ...
    checkbox({
      ...
      field: `inline_color`,
      help: html` If checked, unit names will be color-coded in texts. `,
    }),
    checkbox({
    ...
    checkbox({
      ...
      help: html`
        If checked, then unit skills are displayed as their total sum, instead
        of xx + xx. E.g., instead of 11 ${setup.DOM.Text.successlite(" + 4")}
        ${setup.skill.combat.rep()}, it will display as
        <span data-tooltip="11 + 4">${setup.DOM.Text.successlite("15")}</span>
        ${setup.skill.combat.rep()} (You can hover over it to see the "11 + 4").
      `,
    }),
    checkbox({
    ...
    checkbox({
      ...
      help: html`
        If checked, skin traits such as ${setup.trait.body_werewolf.rep()} or
        ${setup.trait.ears_elf.rep()} are hidden in unit cards.
      `,
    }),
    html`
      <hr />
  ...
  function set_difficulty(new_value: Settings["challengemode"]) {
            ...
            <div class="helpcard">
              ...
              <div>
                ...
                ${setup.DOM.create(
                  "span",
                  {},
                  objectKeys(setup.Settings.DIFFICULTIES).map((diff) =>
                    set_difficulty(diff),
                  ),
                )}
              </div>
...
export const DOM_Menu_settingsmain = function (): DOM.Node {
  ...
  fragments.push(html`
    ...
    <div>
      ${setup.DOM.Nav.link("Manage Global Settings, Imagepacks & Mods", () =>
    ...
    <div>
      ...
      ${setup.DOM.Util.help(html`
        ...
        This is useful for mobile users whose devices does not support the "Save
        to Disk" and "Load to Disk" options in the game, for whom these buttons
        will not be visible.
      `)}
    </div>
    <div>
  ...
  if (State.variables.gDebug) {
    ...
    fragments.push(html`
      <div>
        ${setup.DOM.Nav.link("(turn on debug mode)", () => {
        ...
        debugging, and turning it on in an ordinary game may have unforeseen
        consequences!
      </div>
    `);
  }
...
export const DOM_Menu_settingsdebug = function (): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html`
    <div>
