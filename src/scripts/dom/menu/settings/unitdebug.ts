...
export const DOM_Menu_unitdebug = function (unit: Unit): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html`
    <div>
      Change ${unit.rep()}
      ${setup.DOM.Nav.link("(Done)", () => {
  ...
  const debug_actions = {
    ...
    "Gender swap": setup.qc.Function(() => {
      const poolkey = (unit.getSubrace().key +
        "_" +
        (unit.getGender() == setup.trait.gender_male
          ? "female"
          : "male")) as UnitPoolKey;
  ...
  function callback(action_key: keyof typeof debug_actions) {
    return () => {
  ...
  const inner = [];
  for (const action_key of objectKeys(debug_actions)) {
    inner.push(html`
      ${setup.DOM.Nav.link(`(${action_key})`, callback(action_key))}
    `);
  }
  ...
  fragments.push(html`
    <div>
      ...
      ${setup.DOM.Nav.link(`(Remove title)`, () => {
        setup.DevToolHelper.pickTitle(
          State.variables.titlelist.getAllTitles(unit),
        ).then((title) => {
          if (title) {
...

  {
    fragments.push(html`
      <div>
        ${setup.DOM.Util.message("(Set skills)", () => {
          const frags = [];
      ...
      ${setup.DOM.Util.message(
        ...
        html`
          <div class="helpcard">
            ...
            ${setup.DOM.Nav.button(`Change`, () => {
              unit.setName(State.temporary.first_name, State.temporary.surname);
              refresh();
            })}
          </div>
        `,
  ...
  fragments.push(html`
    <div>
      <div><b>Remove trait</b>:</div>
      ${setup.DOM.create(
        "div",
        {},
        unit.getRemovableTraits().map((trait) =>
          setup.DOM.Nav.link(trait.rep(), () => {
    ...
    <div>
      <div>
        ...
        ${setup.DOM.Menu.traitpickersingle({
          raw_traits: Object.values(setup.trait).filter(
            (trait) => !trait.getTags().includes("computed"),
          ),
          finish_callback: (trait) => {
