...
function traitFragment(
  ...
  if (is_critical) {
    inner.push(html`
      ...
      ${setup.DOM.Util.help(html`
        If the primary unit have these traits then they will have a greater
        chance of doing this activity over others.
      `)}
    `);
  } else {
    inner.push(html`
      ...
      ${setup.DOM.Util.help(html`
        If the primary unit have these traits then they will have a smaller
        chance of doing this activity over others.
      `)}
    `);
  }
  ...
  inner.push(
    setup.DOM.Nav.link(`(change)`, () => {
      setup.DevToolHelper.pickTraits(
        undefined,
        /* init traits = */ current,
      ).then((selected_traits) => {
        let varname;
        if (is_critical) {
          varname = "critical_trait_keys" as const;
        } else {
          varname = "disaster_trait_keys" as const;
        }
...
function dialogueEditFragment(
  ...
  if (dialogue_index < dialogues.length - 1) {
    movedown = setup.DOM.Nav.link(`(move down)`, () => {
      dialogues.splice(
        dialogue_index,
        0,
        dialogues.splice(dialogue_index + 1, 1)[0],
      );
      refreshPage();
    });
  }
  ...
  let moveup = null;
  if (dialogue_index > 0) {
    moveup = setup.DOM.Nav.link(`(move up)`, () => {
      dialogues.splice(
        dialogue_index - 1,
        0,
        dialogues.splice(dialogue_index, 1)[0],
      );
      refreshPage();
    });
  }
  ...
  fragments.push(html`
    <div>
      Dialogue sequence <b>#${dialogue_index + 1}</b> by actor
      ${twee`<<devactor '$dtquest.dialogues[${dialogue_index}].actor'>>`}
      ${moveup} ${movedown} ${remove}
      ${setup.DOM.Util.help(html`Click to change the actor`)}
    </div>
  `);

  for (const perkey of objectKeys(dialogue.texts)) {
    const inner = [];
    ...
    if (texts.length >= 2) {
      ...
      for (let i = 0; i < texts.length; ++i) {
        veryin.push(
          setup.DOM.Nav.link(`(remove alternate text #${i + 1})`, () => {
    ...
    inner.push(html`
      <div>
        Dialogues for ${setup.DOM.Text.successlite(setup.capitalize(perkey))}
        units:
        ${setup.DOM.Nav.link(`(Add alternate text)`, () => {
          dialogue.texts[perkey].push("");
          refreshPage();
        })}
        ${remove_node}
        ${setup.DOM.Util.help(html`
          If a dialogue has multiple options, one of these will be chosen at
          random.
        `)}
      </div>
    `);
  ...
  return setup.DOM.create("div", { class: "dialoguecard card" }, fragments);
}

function roomSelectFragment(template: ActivityTemplate): DOM.Node {
  const fragments: DOM.Attachable[] = [];

  function selectRoomCallback(room: RoomTemplate) {
    return () => {
  ...
  fragments.push(html`
    <div>
      ...
      ${setup.DOM.Util.help(
        html`This activity will only take place in one of these rooms`,
      )}
      ${setup.DOM.Nav.move("(add new)", "AGChooseRoom")}
    </div>
  `);

  function removeRoomCallback(room: RoomTemplate) {
    return () => {
      template.room_template_keys = template.room_template_keys.filter(
        (key) => key != room.key,
      );
      refreshPage();
    };
  ...
  for (const room_template of template.getRoomTemplates()) {
    fragments.push(html`
      <div>
        ${room_template.rep()}
        ${setup.DOM.Nav.link(`(remove)`, removeRoomCallback(room_template))}
      </div>
    `);
  }
...
export const DOM_Menu_devtoolactivity = function (
  template: ActivityTemplate,
): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  ...
  fragments.push(
    html`<div>
      Write your activity dialogues below. You can either have different
      dialogue per personality, or you can
      ${setup.DOM.Util.message(
        `use the same dialogue for all personalities`,
        html`<div class="helpcard">
          Simply fill out only the "Friendly" dialogue and leave the rest
          completely blank to achieve this effect.
        </div>`,
      )}
    </div>`,
  );
  ...
  fragments.push(html`
    <div>
      ${setup.DOM.Nav.link(`(add new dialogue sequence)`, () => {
        template.dialogues.push(
          setup.DialogueHelper.createEmptyDialogue(
            dialogues[dialogues.length - 1].actor,
          ),
        );
        refreshPage();
      })}
...
export const DOM_Menu_devtoolactivityselectroom = function (
  template: ActivityTemplate,
): DOM.Node {
  const fragments: DOM.Attachable[] = [];
  fragments.push(html` <p>Choose a room:</p> `);

  const current = template.getRoomTemplates();
  const all_rooms = Object.values(setup.roomtemplate).filter(
    (room) => !current.includes(room),
  );
