function format_skill(value: number): DOM.Node {
  if (value > 0) {
...
function delineate_fragments(fragments: DOM.Node[]): DOM.Node {
  const frags = [];
  for (let i = 0; i < fragments.length; ++i) {
    if (i) frags.push(html` | `);
    frags.push(fragments[i]);
  }
  return setup.DOM.create("div", {}, frags);
}
...
export default {
  unitskill(unit: Unit, skill: Skill): DOM.Node {
    ...
    fragments.push(html`
      <div>${unit.rep()}'s ${skill.rep()}: ${unit.getSkill(skill)}</div>
      <hr />
    `);
    ...
    fragments.push(html` <div>Base: ${format_skill(skill_base)}</div> `);
    ...
    fragments.push(
      delineate_fragments(
        base_breakdown.map(
          (breakdown) =>
            html`${breakdown.title}: ${format_skill(breakdown.value)}`,
        ),
      ),
    );
    ...
    fragments.push(html` <div>Mods: ${setup.DOM.Text.percentage(mods)}</div> `);
      ...
      delineate_fragments(
        mod_breakdown.map(
          (breakdown) =>
            ...
            ${setup.DOM.Text.percentage(breakdown.value)}`,
        ),
      ),
    );
    fragments.push(html`<hr />`);
    ...
    fragments.push(html` <div>Add: ${format_skill(adds)}</div> `);
