...
export default {
  ...
  tooltipunitstatus(unit: Unit): DOM.Node {
    ...
    if (quest) {
      fragments.push(html` <div>Busy with ${quest.rep()}</div> `);
    }

    const opportunity = unit.getOpportunity();
    if (opportunity) {
      fragments.push(html` <div>Busy with ${opportunity.rep()}</div> `);
    }

    const is_on_leave = State.variables.leave.isOnLeave(unit);
    if (is_on_leave) {
      fragments.push(html` <div>${setup.DOM.Card.leave(unit)}</div> `);
    }

    const is_injured = unit.isInjured();
    if (is_injured) {
      fragments.push(html`
        <div>Injured: ${setup.DOM.Card.injury(unit)}</div>
      `);
    }

    const duty = unit.getDuty();
    if (duty) {
      fragments.push(html` <div>Duty: ${duty.rep()}</div> `);
    }
