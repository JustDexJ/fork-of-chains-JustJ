...
function slaveOrderMenuItems(unit: Unit, slave_order: SlaveOrder): JQuery[] {
  const menus = [];
    ...
    if (unit.getParty()) {
      menus.push(
        menuItemText({
          text: html`
            <span
              data-tooltip="You cannnot sell units that are currently in a party. Remove them from the party if you wish to sell them."
            >
      ...
      menus.push(
        menuItemText({
          text: html`
            <span
              data-tooltip="A unit can only fulfill a slave order if they would give you at least 1g. Some quests would require the slave to have several of the critical traits in order to do so."
            >
  ...
  fragments.push(
    setup.DOM.Card.criteriatraitlist(
      slave_order.getCriteria(),
      unit,
      /* ignore extra = */ true,
    ),
  );
