export function TextUnit_background(unit: Unit): string {
  let backgrounds = unit
    .getTraits()
    .filter((trait) => trait.getTags().includes("bg"));
  ...
  for (let i = 0; i < backgrounds.length; ++i) {
    ...
    if (bg == setup.trait.bg_farmer) {
      ...
      if (
        ...
        race == setup.trait.race_greenskin ||
        subrace == setup.trait.subrace_humandesert
      ) {
      ...
      if (unit.isHasTrait(setup.trait.per_lustful) || unit.isMasochistic()) {
        ...
        slavertext =
          "Perhaps this sluttiness can come in handy for slaving career?";
      } else {
        slavertext = `${basetext}.\
