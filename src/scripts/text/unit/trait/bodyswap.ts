...
export namespace TextUnitBodyswap {
  /**
   * Describe what happens when unit went from oldtrait to newtrait
   */
  export function traitTransform(
    unit: Unit,
    oldtrait: Trait | null,
    newtrait: Trait | null,
  ): string | string[] {
    ...
    if (!trait_group || !trait_group.isOrdered()) {
      ...
      let idxold = trait_group._getTraitIndex(oldtrait);
      let idxnew = trait_group._getTraitIndex(newtrait);
      ...
      if (idxold < idxnew) {
        ...
        if (newtrait) plus = newtrait.text().increase ?? plus;
      } else {
        // decrease
        if (tags.includes("muscle")) {
        ...
        if (newtrait) plus = newtrait.text().decrease ?? plus;
      }
      return `${initsentence}${plus}.`;
    }
  }

  export function bodyswap(unit1: Unit, unit2: Unit): string {
    let comptraits = ["gender", "subrace"]
      .concat(setup.TRAIT_PHYSICAL_TAGS)
      .concat(setup.TRAIT_SKIN_TAGS);
    let texts = [];
    for (let i = 0; i < comptraits.length; ++i) {
      let comptrait = comptraits[i];
      let text = traitTransform(
        unit1,
