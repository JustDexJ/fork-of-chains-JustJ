...
export namespace TextUnitEquipment {
  export function getEquipmentSetFallback(unit: Unit): EquipmentSet {
    let equipmentset = unit.getEquipmentSet();
    if (!equipmentset) {
   ...
   */
  export function getWeapon(unit: Unit): Equipment | null {
   ...
   */
  export function getWeaponRep(unit: Unit): string {
   ...
   */
  export function getWeaponRepFull(unit: Unit): string {
    const weapon = setup.Text.Unit.Equipment.getWeapon(unit);
    if (weapon) {
      let base = weapon.rep();
   ...
   */
  export function getAWeaponRep(unit: Unit): string {
    const weapon = setup.Text.Unit.Equipment.getWeapon(unit);
    if (weapon) {
  ...
  export function isNaked(unit: Unit): boolean {
    return (
      !setup.Text.Unit.Equipment.getEquipmentAt(
        unit,
  ...
  export function isFaceCovered(unit: Unit): boolean {
    let equipment = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
  ...
  export function isChestCovered(unit: Unit): boolean {
    let equipment = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.torso,
    );
    return (
      !!equipment &&
      equipment.getSluttiness() < setup.EQUIPMENT_SLUTTY_COVERED_THRESHOLD
    );
  }

  export function isGenitalCovered(unit: Unit): boolean {
    // outer layer
    let equipment = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.legs,
    ...
    // underwear
    equipment = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
  ...
  function describeEquipment(
    ...
    let their = setup.DOM.PronounYou.their(unit);
    let Their = `<<Their "${unit.key}">>`;
    let they = `<<they "${unit.key}">>`;
    let They = `<<They "${unit.key}">>`;
    let them = `<<them "${unit.key}">>`;

    let eqrep = equipment.rep();

    // special things
    let tags = equipment.getTags();
    ...
    if (tags.includes("hat")) {
      ...
      let base = ` which is entirely covered by ${eqrep}`;
      if (tags.includes("pet"))
        base += ` complete with leather pet ears and muzzle`;
      if (tags.includes("pony"))
        base += ` complete with leather pony ears and muzzle`;
      return base;
    ...
    /* Dick */
    if (
      slot == setup.equipmentslot.genital &&
      tags.includes("dick") &&
      tags.includes("chastity")
    ) {
    ...
    /* generic ones */
    let sluttiness = equipment.getSluttiness();
    let verb = "";
    let afterword = "";
    ...
    if (tags.includes("armor")) {
      if (sluttiness < setup.EQUIPMENT_SLUTTY_COVERED_THRESHOLD) {
        ...
        verb = "protected";
        afterword =
          "although its questionable how much protection it gives given the amount of skin it actually cover";
      }
    ...
    let base = ` which is ${verb} by ${their} ${eqrep}`;
    if (afterword) base += ` ${afterword}`;

    if (
      slot == setup.equipmentslot.torso &&
      unit.isHasTrait(setup.trait.muscle_verystrong) &&
      equipment.isCovering()
    ) {
  ...
  export function slot(unit: Unit, slot: EquipmentSlot) {
    ...
    let equipment = setup.Text.Unit.Equipment.getEquipmentAt(unit, slot);
    let their = `<<their "${unit.key}">>`;
    let Their = `<<Their "${unit.key}">>`;
    let they = `<<they "${unit.key}">>`;
    let They = `<<They "${unit.key}">>`;
  ...
  export function equipmentSummary(unit: Unit) {
    let pertraits = unit.getAllTraitsWithTag("equipment");
    let adjectives = [];
    ...
    if (
      pertraits.includes(setup.trait.eq_restrained) ||
      pertraits.includes(setup.trait.eq_blind)
    ) {
      props.push("bondage");
    }
    ...
    if (setup.Text.Unit.Equipment.isNaked(unit)) {
      ...
      let torso = setup.Text.Unit.Equipment.getEquipmentAt(
        unit,
        setup.equipmentslot.torso,
      );
      if (!torso) {
  ...
  export function stripTorso(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);
    ...
    if (torso.getTags().includes("harness")) {
      ...
      return (
        `${They} hurriedly attempted to get out of ${their} protective ${torso.rep()}, ` +
  ...
  export function stripLegs(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);

    let legs = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.legs,
    );
    let underwear = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.rear,
    );
    ...
    if (underwear) {
      ...
      if (underwear.getTags().includes("harness")) {
        ...
        text += `${They} "strips out" of ${their} "invisible" ${underwear.rep()}, before making a gesture throwing it away,`;
  ...
  export function stripVagina(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);

    let vagina = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.genital,
    );
    if (!vagina) return ""; // nothing to strip
    let tags = vagina.getTags();
  ...
  export function stripDick(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);
    let them = setup.DOM.PronounYou.them(unit);

    let dick = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.genital,
    );
    if (!dick) return ""; // nothing to strip
    let tags = dick.getTags();
  ...
  export function stripNipple(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);

    let nipple = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.nipple,
    );
    if (!nipple) return ""; // nothing to strip
    let tags = nipple.getTags();
  ...
  export function stripGenital(unit: Unit) {
    if (unit.isHasDick()) {
  ...
  export function stripAnus(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);

    let anus = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.rear,
    );
    if (!anus) return ""; // nothing to strip
    let tags = anus.getTags();
  ...
  export function youStripAnus(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);
  ...
  export function stripMouth(unit: Unit) {
    let their = setup.DOM.PronounYou.their(unit);
    let Their = setup.DOM.PronounYou.Their(unit);
    let they = setup.DOM.PronounYou.they(unit);
    let They = setup.DOM.PronounYou.They(unit);

    let mouth = setup.Text.Unit.Equipment.getEquipmentAt(
      unit,
      setup.equipmentslot.mouth,
    );
    if (!mouth) return ""; // nothing to strip
    let tags = mouth.getTags();
  ...
  export function slaverStripAll(unit: Unit) {
    if (setup.Text.Unit.Equipment.isNaked(unit)) return "";
    let eq = setup.Text.Unit.Equipment.equipmentSummary(unit);
