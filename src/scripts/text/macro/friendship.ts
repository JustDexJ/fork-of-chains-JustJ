...
const getRel = (unit1: Unit, unit2: Unit) => {
...
const getTheirRel = (unit1: Unit, unit2: Unit): string => {
...
const getNameRel = (unit1: Unit, unit2: Unit): string => {
  let rel = getRel(unit1, unit2);
  if (!rel) return "";
  return `${unit1.rep()} ${rel}`;
};

function internalOutput<T>(
