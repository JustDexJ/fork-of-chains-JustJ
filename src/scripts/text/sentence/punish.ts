export namespace TextPunish {
  export function getPunishReasonSlave(unit: Unit) {
    let their = `<<their "${unit.key}">>`;
    let they = `<<they "${unit.key}">>`;
    let them = `<<them "${unit.key}">>`;
    let themselves = `<<themselves "${unit.key}">>`;
    let rep = unit.rep();
  ...
  export function getPunishReasonSlaver(unit: Unit) {
    let their = `<<their "${unit.key}">>`;
    let they = `<<they "${unit.key}">>`;
    let them = `<<them "${unit.key}">>`;
    let themselves = `<<themselves "${unit.key}">>`;
    ...
    if (unit.isMasochistic()) {
      outputs.push(
        `the masochistic slaver ${rep} showed ${their} masochistic advances`,
      );
    }
  ...
  export function punishreason(unit: Unit) {
    let outputs = [];

    if (unit.isSlave()) {
      outputs = getPunishReasonSlave(unit);
    } else {
      outputs = getPunishReasonSlaver(unit);
    }
