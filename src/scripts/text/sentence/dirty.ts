...
export namespace TextDirty {
  export interface SexDialogue {
    unit_dialogue: string;
    target_dialogue: string;
  }

  function noPenetrationDialogue(
    me: Unit,
    them: Unit,
    sex: SexInstance,
  ): SexDialogue {
    ...
    if (me_speech == setup.speech.witty) {
      ...
      if (
        [setup.sexpace.dom, setup.sexpace.normal].includes(me_pace) &&
        me.isSlaver() &&
        them.isSlaver()
      ) {
        /*
    ...
    return {
      unit_dialogue: setup.SexUtil.convert(
        me_dialogue,
        { a: me, b: them },
        sex,
      ),
      target_dialogue: setup.SexUtil.convert(
        them_dialogue,
        { a: me, b: them },
        sex,
      ),
  ...
  export function talk({
    ...
    if (!sex) {
      ...
      sex = new setup.SexInstance(
        setup.sexlocation.dungeonsfloor,
        participants,
      );
      ...
      if (unit_bodypart!.isCanPenetrate(target_bodypart!)) {
        ...
        throw new Error(
          `${unit_bodypart!.key} cannot interact with ${target_bodypart!.key}`,
        );
      }
      if (unit_pace) {
   ...
   * me says something
   */
  function yesPenetrationDialogue(
    me: Unit,
    them: Unit,
    penetration: UnitAndBodypart,
    sex: SexInstance,
  ): string {
    if (penetration.unit != me) {
      penetration = sex.getBodypartOther(
        penetration.unit,
        penetration.bodypart,
      )!;
    }
    if (penetration.unit != me)
      throw new Error(`??? penetration unit is not me?`);
    const me_bodypart = penetration.bodypart;
    const them_bodypart = sex.getBodypartOther(
      penetration.unit,
      penetration.bodypart,
    )!.bodypart;
    ...
    let fuck;
    if (
      sex.getBodypartPenetrationTarget(penetration.unit, penetration.bodypart)
    ) {
      // i am penetrating
      is_penetrated = false;
    ...
    if (is_penetrated) {
      ...
      if (me_pace == setup.sexpace.normal) {
        ...
        if (me_bodypart == setup.sexbodypart.penis) {
          texts.push(`Take my entire length! Take it all in!`);
        }
      } else if (me_pace == setup.sexpace.dom) {
        texts.push(
