export function getMasterNickname(unit: Unit, target: Unit): string {
  if (target.isMale()) {
    return `master`;
  } else {
    return `mistress`;
...
export namespace TextGreeting {
  ...
  export function nickname({
    ...
    if (
      [
        setup.speech.bold,
        setup.speech.friendly,
        setup.speech.witty,
        setup.speech.debauched,
      ].includes(speech)
    ) {
      if (target.isYou()) {
        names.push(`boss`, `chief`);
      }
    ...
    if (
      [setup.speech.bold, setup.speech.witty, setup.speech.debauched].includes(
        speech,
      )
    ) {
      if (target.isYou()) {
      ...
      if (
        target.isHasTrait("breast_large") ||
        target.isHasTrait("dick_large")
      ) {
        names.push(`sexy`);
      }
    ...
    if (unit.getLover() == target) {
      ...
      if (speech == setup.speech.friendly) {
        names.push(`honey`, `darling`, `sugar`, `dearest`);
      } else if (speech == setup.speech.bold) {
        names.push(`baby`, `hun`, `bunny`, `angel`);
        ...
        names.push(`hottie`, `sexy`, `tasty`, `delicious`);
        if (target.isMale()) {
          names.push(`hunk`, `handsome`);
        } else {
          names.push(`your sexiness`, `gorgeous`);
        }
      } else {
    ...
    return setup.Text.replaceUnitMacros(names[seed % names.length], {
      a: unit,
      b: target,
    });
  }
  ...
  export function short({
    ...
    if (speech == setup.speech.friendly) {
      ...
      greeting_templates.push(`Hey ${tnick}, `, `...${tnick},`, `...,`);
    } else if (speech == setup.speech.witty) {
      greeting_templates.push(
      ...
      if (unit.isFemale()) {
        greeting_templates.push(`Ara, it's ${tnick},`);
      }
    } else {
      throw new Error(`Unrecognized speech pattern: ${speech.key}`);
    }
    ...
    if (unit.isHasTrait("bg_farmer")) {
      if (speech != setup.speech.debauched) {
        greeting_templates.push(`Howdy partner,`, `Howdy pardner,`);
      }
    }
    ...
    return setup.Text.replaceUnitMacros(chosen, { a: unit, b: target });
  }
