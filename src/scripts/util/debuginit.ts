...
import type { JobKey } from "../classes/job/Job";
import type { OpportunityTemplate } from "../classes/opportunity/OpportunityTemplate";
import type { Rotation } from "../classes/room/RoomInstance";
...
export namespace DebugInit {
   ...
   */
  export function initBuildings(is_fast?: boolean) {
    const sv = State.variables;
    /* Construct buildings */
    let buildings;
    if (is_fast) {
  ...
  function initRooms() {
    ...
    for (const to_upgrade of upgrades) {
      State.variables.fort.player
        .getBuilding(setup.buildingtemplate[to_upgrade])!
        .upgrade();
    }
    const room_locations = {
      ...
      innroom: [1, 5, 22],
      garden: [0, 21, 8],
    } as Record<RoomTemplateKey, [Rotation, number, number]>;

    for (const room of Object.values(State.variables.roominstance)) {
      const template_key = room.getTemplate().key;
      if (template_key in room_locations) {
        const loc = room_locations[template_key];
        room.setRotation(loc[0]);
        State.variables.fortgrid.relocateRoom(room, {
          row: loc[1],
          col: loc[2],
        });
      }
  ...
  export function initSlavesCommon() {
    const sv = State.variables;
    ...
    for (const unit of to_train) {
      ...
      for (const trait of TraitHelper.getAllTraitsOfTags([
        "training",
        "tradvanced",
      ])) {
  ...
  export function contentCreatorInit() {
    console.info("Starting content creator init");
  ...
  export function getEquipmentSet(equipments: Equipment[]) {
    const sv = State.variables;
  ...
  export function debugModeInit(hideDebugInfo?: boolean) {
    ...
    /* Special units */
    {
      const units = sv.company.player.getUnits(
        { job: setup.job.slaver },
        "name",
      );
      const slaves = sv.company.player.getUnits(
        { job: setup.job.slave },
        "name",
      );

      /* Traumatized slaver */
      console.info("traumatizing slavers");
      sv.trauma.traumatize(units[0], 10);
      sv.trauma.traumatize(units[0], 10);

      /* Boonize slaver */
      console.info("boonizing slavers");
      sv.trauma.boonize(units[1], 10);
      sv.trauma.boonize(units[1], 10);

      /* Best friends and arch rivals */
      console.info("friendship / rivalry slavers");
    ...
    {
      console.info("market objects");
      setup.qc
        .EquipmentForSale(setup.equipmentpool.all_combat, /* amount = */ 8)
        .apply(setup.costUnitHelper(null, "debug"));
      setup.qc
        .EquipmentForSale(setup.equipmentpool.all_sex, /* amount = */ 8)
        .apply(setup.costUnitHelper(null, "debug"));
      setup.qc
        .ItemForSale("itemmarket", setup.itempool.all, /* amount = */ 8)
        .apply(setup.costUnitHelper(null, "debug"));
      setup.qc
        .ItemForSale(
          "itemmarket",
    ...
    {
      console.info("equipments initializations");
      //... just get all items.
    ...
    /* Bedchambers */
    {
      ...
      for (const bc of [bc1, bc2]) {
        ...
        if (bc == bc1)
          sv.friendship.adjustFriendship(totrain, bc.getSlaver(), 1000);
        if (bc == bc1) State.variables.family.setTwin(maleslave, femaleslave);
        if (bc == bc1)
          State.variables.family.setSibling(bc.getSlaver(), femaleslave);
      }
    ...
    /* slave orders */
    {
      console.info("slave orders");
      setup.qc.SlaveOrderCapitalOfSlaves().apply({ getActorUnit: () => null });
    }

    /* Favor, ire */
    {
      console.info("favor / ire");
      setup.qc.Favor("neko", 600).apply();
      setup.qc.Ire("bank", 10).apply();
    }

    /* Level up units */
    console.info("level up units");
    levelUp();

    console.info("pop all");
    sv.notification.popAll();

    console.info("end of debug init");
    ...
    /* Check if any quest is missing location tag */
    {
      ...
      for (const to_check of to_checks) {
        for (const obj of Object.values(to_check) as (
          | QuestTemplate
          | OpportunityTemplate
        )[]) {
          if (!obj.getTags().filter((tag) => locations.includes(tag)).length) {
            console.info(`${obj.key} is missing a location tag.`);
          }
        }
      }
    }
