const tags_to_brackets: Record<string, number> = {
  if: 2,
  else: 2,
  elseif: 2,
  switch: 2,
...
const tag_mid: Record<string, string> = {
  else: "if",
  elseif: "if",
  opt: "choose",
  optif: "choose",
...
function tagpretty(tag: string, is_closing: boolean): string {
  if (!tag) return "undefined";
  const expected = tags_to_brackets[tag];
  if (expected === undefined)
    throw new Error(`Undefined expected brackets for tag ${tag}`);
  return (
    "<".repeat(expected) + (is_closing ? "/" : "") + tag + ">".repeat(expected)
  );
}

interface BeautifyError {
  error: string;
  line: number;
  context: string | null;
}
 ...
 * Beautify a string.
 */
export function beautifyTwine(s: string): {
  errors: Array<BeautifyError>;
  text: string;
} {
  if (!s) {
    return {
  ...
  const errors: BeautifyError[] = [];

  function build_context(idx: number) {
    let j = idx + 1;
