import { QuestTemplate } from "../classes/quest/QuestTemplate";
...
function makeIfElseBlocks(conditions: string[], add_else?: boolean) {
  if (conditions.length === 0) return "";

  let str = `<<if ${conditions[0]}>>\n\n`;
  for (let i = 1; i < conditions.length; ++i)
    str += `<<elseif ${conditions[i]}>>\n\n`;
  if (add_else) str += `<<else>>\n\n`;
  return (str += "<</if>>\n");
}

function makeGetSeedBlocks(seeds: number) {
  if (seeds == 0) return "";
  let tstr = "";
...
function makeIfBlocks(conditions: string[]) {
  if (conditions.length === 0) return "";
...
interface CodeEditorMacroAction {
  text: string;
...
const ACTOR_MACROS_INFO: CodeEditorMacroAction[] = [
  {
    text: "Bob / Alice / you",
    output: (key) => `<<rep ${key}>>`,
...
const ACTOR_VERB: CodeEditorMacroAction[] = [
  {
    replace: true,
    text: "is / are",
    output: (key) => `${key}|is`,
...
const ACTOR_OBJECT: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_TERMS: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_PRONOUN: CodeEditorMacroAction[] = [
  {
    text: "he / she / you",
    output: (key) => `<<they ${key}>>`,
...
const ACTOR_MACROS_BODY_HEAD: CodeEditorMacroAction[] = [
  {
    text: "head",
...
const ACTOR_MACROS_BODY_UPPER: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_BODY_LIMBS: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_BODY_NETHERS: CodeEditorMacroAction[] = [
  {
  ...
  },
  {
    ...
    tooltip:
      'Will either become a "dick" or "large dick" if the unit has a dick, or "strap-on" or "fake dick" otherwise.',
  },
  {
...
const ACTOR_MACROS_EQUIP: CodeEditorMacroAction[] = [
  {
    text: "weapon (e.g., Axe)",
...
const ACTOR_MACROS_STRIP: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_TEXT: CodeEditorMacroAction[] = [
  {
...
const ACTOR_MACROS_DIALOGUE: CodeEditorMacroAction[] = [
  {
...
<</dialogue>>`,
  ...
  },
  {
    ...
    tooltip:
      "Returns a full sentence describing an actor greeting you. Can alternatively receive two parameters, where the second parameter is the master.",
  },
  {
...
const ACTOR_MACROS_DIALOGUE_SLAVE: CodeEditorMacroAction[] = [
  {
    text: "Yes, master.",
    output: (key) => `<<uyesmaster ${key}>>`,
    tooltip:
      "Returns a full sentence describing an slave obeying their master.",
  },
  {
    text: "Yelp...! (pet whine)",
...
const ACTOR_MACROS_IF_HAS: CodeEditorMacroAction[] = [
  {
    text: "Has dick?",
...
const ACTOR_MACROS_IFS: CodeEditorMacroAction[] = [
  {
    text: "Submissive?",
...
const ACTOR_MACROS_INLINE_EFFECT: CodeEditorMacroAction[] = [
  {
    replace: true,
    text: "Injure",
  ...
  },
  {
    ...
    output: (key) =>
      `<<run setup.qc.TraumatizeRandom('${key}', 5).apply($gQuest)>>`,
  },
  {
    replace: true,
    text: "Boonize",
    ...
    output: (key) =>
      `<<run setup.qc.BoonizeRandom('${key}', 5).apply($gQuest)>>`,
  },
  {
    replace: true,
    text: "Corrupt",
  ...
  },
  {
    ...
    output: (key) =>
      `<<run setup.qc.Blessing('${key}', 1, null, true).apply($gQuest)>>`,
  },
  {
    replace: true,
    ...
    output: (key) =>
      `<<run setup.qc.FriendshipWithYou('${key}', 50).apply($gQuest)>>`,
  },
  {
    replace: true,
...
function makeActorLabel(actor_key: string): string {
  return actor_key === "Player" ? actor_key : "<i>" + actor_key + "</i>";
}

function getRoleTexts(): string {
  const qbase = State.variables.dtquest;
  if (qbase instanceof QuestTemplate) {
...
function getTraitArrayText(trait_array: Trait[]): string {
  return "[" + trait_array.map((trait) => `'${trait.key}'`).join(", ") + "]";
}

export function generateCodeEditorToolbarItems(
  retainEditorFocus: <T>(promise: Promise<T>) => Promise<T>,
) {
  function makeActorMenu(macros: CodeEditorMacroAction[]) {
    let items = [];
        ...
        menuItem({
          ...
          children: () => [
            menuItem({
              ...
              callback: () => {
                retainEditorFocus(setup.DevToolHelper.pickTraits()).then(
                  (traits) => {
                    if (traits && traits.length) {
                      const role_text = getRoleTexts();
                      for (const trait of traits) {
                        insertTextIntoEditor(
                          `<<set _unit = setup.selectUnit([${role_text}], {trait: '${trait.key}'})>>\n`,
                        );
                        insertTextIntoEditor(
...
<</if>>`,
            ...
            menuItem({
              ...
              callback: () => {
                const qbase = State.variables.dtquest! as QuestTemplate;
                const roles = qbase.getUnitCriterias();
                const notyou = [];
                ...
                const role_text = Object.keys(roles)
                  .map((key) => `$g.${key}.isYou()`)
                  .join(" or ");
  ...
  const toolbar_items = [
    ...
    menuItem({
      ...
      children: () => [
        menuItem({
          text: "Nethers",
          children: () => makeActorMenu(ACTOR_MACROS_BODY_NETHERS),
        }),
        menuItem({
          text: "Limbs",
          children: () => makeActorMenu(ACTOR_MACROS_BODY_LIMBS),
        }),
        menuItem({
          text: "Upper body",
          children: () => makeActorMenu(ACTOR_MACROS_BODY_UPPER),
        }),
        menuItem({
          text: "Head",
          children: () => makeActorMenu(ACTOR_MACROS_BODY_HEAD),
        }),
      ],
    }),
    menuItem({
      ...
      children: () => [
        ...
        menuItem({
          ...
          children: () => [
            menuItem({
              ...
              children: () =>
                setup.DevToolHelper.getActors().map(([actor_key, varname]) =>
                  menuItem({
                    text: makeActorLabel(actor_key),
                    cssclass: "submenu-actor",
                    callback: () => {
        ...
        ...makeActorMenu([
          ...
          },
          {
            ...
            output: (unit_key) =>
              makeIfElseBlocks(
                Object.keys(setup.Text.Race.REGIONS).map((key) => {
    ...
    menuItem({
      ...
      children: () => [
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickCompany()).then(
              (company) => {
                if (company) {
        ...
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickBuilding()).then(
              (building) => {
                if (building)
                  insertTextIntoEditor(
                    `<<rep setup.buildingtemplate.${building.key}>>`,
                  );
              },
            );
          },
        ...
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickItem()).then((item) => {
              if (item) insertTextIntoEditor(`<<rep setup.item.${item.key}>>`);
            });
          },
        }),
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickEquipment()).then(
              (equipment) => {
                if (equipment)
                  insertTextIntoEditor(
                    `<<rep setup.equipment.${equipment.key}>>`,
                  );
              },
            );
          },
        ...
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickLore()).then((lore) => {
              if (lore) insertTextIntoEditor(`<<lore ${lore.key}>>`);
            });
          },
        }),
        menuItem({
        ...
        menuItem({
          ...
          callback: () => {
            retainEditorFocus(setup.DevToolHelper.pickDutyTemplate()).then(
              (template) => {
                if (template)
    ...
    menuItem({
      ...
      children: () => [
        ...
        menuItem({
          text: "Favor",
          tooltip:
            "Formats a favor amount. The example formats 15.0 favor. Note that the amount is multiplied by 10 inside the code.",
          callback: () => {
        ...
        menuItem({
          text: "Font",
          tooltip:
            "Create a segment of text using race font. You can also use a font name instead.",
          children: () =>
            TraitHelper.getAllTraitsOfTags(["subrace"]).map((trait) =>
              menuItem({
                text: trait.rep(),
                callback: () => {
...
<</font>>`);
  ...
  <</fontsize>>`);
        ...
        menuItem({
          text: "Letter card",
          tooltip:
            "Creates a text segment styled to look like a written letter.",
          callback: () => {
  ...
  if (State.variables.devtooltype != "interaction") {
    toolbar_items.push(
      menuItem({
        text: "Conditionals",
        tooltip:
          "Allowing you to show different texts depending on certain non-actor-related conditions",
        children: () => {
          const children = [
            menuItem({
...
<</if>>`);
            ...
            menuItem({
              ...
              callback: () => {
                retainEditorFocus(setup.DevToolHelper.pickItem()).then(
                  (item) => {
                    if (item)
              ...
              menuItem({
                ...
                children: [
                    ...
                    callback: () => {
                      retainEditorFocus(setup.DevToolHelper.pickItem()).then(
                        (item) => {
                          if (item)
                            insertTextIntoEditor(
                              `<<run setup.qc.Item('${item.key}').apply($gQuest)>>`,
                            );
                        },
                      );
                    },
                  ...
                  menuItem({
                    ...
                    callback: () => {
                      retainEditorFocus(
                        setup.DevToolHelper.pickItemPool(),
                      ).then((pool) => {
                        if (pool)
                          insertTextIntoEditor(
                            `<<run setup.qc.ItemPool('${pool.key}').apply($gQuest)>>`,
                          );
                      });
                    },
                  }),
                  menuItem({
                    ...
                    callback: () => {
                      retainEditorFocus(
                        setup.DevToolHelper.pickEquipment(),
                      ).then((equipment) => {
                        if (equipment)
                          insertTextIntoEditor(
                            `<<run setup.qc.EquipmentDirect('${equipment.key}').apply($gQuest)>>`,
                          );
                      });
                    },
                  }),
                  menuItem({
                    ...
                    callback: () => {
                      retainEditorFocus(
                        setup.DevToolHelper.pickEquipmentPool(),
                      ).then((pool) => {
                        if (pool)
                          insertTextIntoEditor(
                            `<<run setup.qc.Equipment('${pool.key}').apply($gQuest)>>`,
                          );
                      });
                    },
                  }),
                ...
                callback: () => {
                  retainEditorFocus(setup.DevToolHelper.pickCompany()).then(
                    (company) => {
                      if (company) {
                        insertTextIntoEditor(
                          `<<run setup.qc.Favor('${company.key}', 25).apply($gQuest)>>`,
                        );
                      }
                    },
                  );
                ...
                callback: () => {
                  retainEditorFocus(setup.DevToolHelper.pickCompany()).then(
                    (company) => {
                      if (company) {
                        insertTextIntoEditor(
                          `<<run setup.qc.Ire('${company.key}', 2).apply($gQuest)>>`,
                        );
                      }
                    },
                  );
              ...
              menuItem({
                ...
                children: () => [
                  menuItem({
                    ...
                    callback: () => {
                      insertTextIntoEditor(
                        `<<run setup.qc.VarSet('put_variable_name_here', '5', -1).apply($gQuest)>>`,
                      );
                    },
                  }),
                  menuItem({
                    ...
                    callback: () => {
                      insertTextIntoEditor(
                        `<<run setup.qc.VarAdd('put_variable_name_here', '3', -1).apply($gQuest)>>`,
                      );
                    },
                  }),
                  menuItem({
                    ...
                    callback: () => {
                      insertTextIntoEditor(
                        `<<run setup.qc.VarRemove('put_variable_name_here').apply($gQuest)>>`,
                      );
                    },
                  }),
                ],
