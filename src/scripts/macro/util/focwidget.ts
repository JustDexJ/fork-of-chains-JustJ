/**
 * Same as SugarCube's "widget", but on it error renders
 * everything instead of just showing an error message
 *
...
Macro.add("focwidget", {
  ...
  handler() {
    ...
    try {
        ...
        handler: (function (contents) {
          return function () {
            ...
            try {
              ...
              return this.error(
                `cannot execute widget: ${ex instanceof Error ? ex.message : String(ex)}`,
              );
            } finally {
      ...
      return this.error(
        `cannot create widget macro "${widgetName}": ${ex instanceof Error ? ex.message : String(ex)}`,
      );
    }
  },
});
