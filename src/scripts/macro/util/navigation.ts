import { reload } from "../../dom/nav/nav.js";
  ...
  handler() {
    ...
    if (this.name == "focreturn") {
      if (this.args.length != 1)
        throw new Error(
          `Incorrect length of arguments for <<focreturn>>: must have exactly 1 element but ${this.args.length} found`,
        );
    } else {
      if (this.args.length != 2)
        throw new Error(`<<focmove>> must have 2 arguments but 1 found`);
...
Macro.add(["focgoto", "focsavestategoto"], {
  handler() {
    ...
    if (
      this.args.length == 1 &&
      State.variables.gPassage != State.variables.gOldPassage
    )
      scroll_top = true;
    reload(
      /* force_passage_switch = */ this.name == "focsavestategoto",
      scroll_top,
    );
  },
});
...
Macro.add(["foclink", "focbutton"], {
  ...
  handler() {
    ...
    if (this.args.length < 1 || this.args.length > 2)
      throw new Error(
        `Incorrect length of arguments for <<${this.name}>>: must have 1-2 elements but ${this.args.length} found`,
      );
    if (this.args.length == 2 && !Story.has(this.args[1])) {
      throw new Error(`Passage ${this.args[1]} not found for <<${this.name}>>`);
    }
